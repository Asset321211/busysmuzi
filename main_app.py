...
class InstrScr(Screen):
    def __init__(self, **kwargs):
        ...
        outer = BoxLayout(orientation='vertical',padding = 8,spacing = 0)
        outer.add_widget(instr)
        outer.add_widget(line1)
        outer.add_widget(line2)
        outer.add_widget(self.btn)
        outer.add_widget(outer)
    def next(self):
            global name
            name = self.in_ame.text 
...
class PulseScr(Screen):
    def __init__(self, **kwargs):
        ...
        lbl_result = Label(text='Введите результат:', haliign='right')
        self.in_result = TextInput(text='0', multiple=False)
